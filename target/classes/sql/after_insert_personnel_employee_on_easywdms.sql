...
BEGIN
    ...
    FROM pcs_personnel_management.tenant_company_mapping tcm
    WHERE tcm.personnel_company_id = NEW.company_id;

    -- Check if the entry exists in pcs_personnel_management.personnel_details
    IF EXISTS (
        SELECT 1
        FROM pcs_personnel_management.personnel_details pd
        WHERE pd.personnel_code = NEW.emp_code
            AND pd.application_name = tenant_application_name
    ) THEN
        -- Update the existing entry
        UPDATE pcs_personnel_management.personnel_details pd
        SET
            pd.active = TRUE,
            pd.tenant_company_mapping_id = tenant_mapping_id
        WHERE
            pd.personnel_code = NEW.emp_code
            AND pd.application_name = tenant_application_name;
    END IF;
END $$
