...
public interface SStaffShiftsRepository extends JpaRepository<SStaffShifts, Long> {
	...
	@Query(value = "SELECT ss.*, s.designation FROM staff_shifts ss join personnel_details s on ss.staff_id = s.personnel_code WHERE ss.tenant_id = ?1 and ss.store_id = ?2 and ss.shift_date = ?3", nativeQuery = true)
